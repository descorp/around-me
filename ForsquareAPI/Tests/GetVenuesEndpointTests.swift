...
final class GetVenuesEndpointTests: XCTestCase {
    ...
    func tesSimpletGetVenueUrl() {
        let sut = Endpoint.getVenues(at: coordinates)
        XCTAssertEqual(try urlBuilder.buildRequest(for: sut).url!.absoluteString,
    "https://api.foursquare.com/v2/venues/search?ll=52.313388,5.037687&radius=250f&client_id=CLIENT_ID&client_secret=CLIENT_SECRET&v=20190804")
    }
    
    func tesOverloadedGetVenueUrl() {
        let sut = Endpoint.getVenues(at: coordinates, radius: 1000, categories: "4bf58dd8d48988d1fd931735", "52f2ab2ebcbc57f1066b8b4f")
            XCTAssertEqual(try urlBuilder.buildRequest(for: sut).url!.absoluteString,
        "https://api.foursquare.com/v2/venues/search?ll=52.313388,5.037687&radius=1000&categoryId=4bf58dd8d48988d1fd931735,52f2ab2ebcbc57f1066b8b4f&client_id=CLIENT_ID&client_secret=CLIENT_SECRET&v=20190804")
    }
    
    func tesOverloaded2GetVenueUrl() {
         let sut = Endpoint.getVenues(at: coordinates, radius: 1000, categories: ["4bf58dd8d48988d1fd931735", "52f2ab2ebcbc57f1066b8b4f"])
             XCTAssertEqual(try urlBuilder.buildRequest(for: sut).url!.absoluteString,
         "https://api.foursquare.com/v2/venues/search?ll=52.313388,5.037687&radius=1000&categoryId=4bf58dd8d48988d1fd931735,52f2ab2ebcbc57f1066b8b4f&client_id=CLIENT_ID&client_secret=CLIENT_SECRET&v=20190804")
     }
